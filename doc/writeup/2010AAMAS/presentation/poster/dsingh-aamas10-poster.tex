...
An important drawback in BDI is the lack of a learning ability. \alert{Context conditions} of plans 
\begin{wrapfigure}{l}{0.5 \columnwidth}
  \begin{center}
    \includegraphics[width=0.45\columnwidth]{figs/mars_rover.jpg}
  \end{center}
\end{wrapfigure}
that determine applicability in given situations,
must be specified upfront. However, new environments often require learning changes to selection conditions.


Easing this constraint would allow conditions to be \alert{refined} over time, improving adaptability. 

Our learning framework models context conditions as \alert{decision trees}, and allow agents to \alert{learn plan applicability} from ongoing experiences. 
...
%%%%% BDI Plan Library
\begin{textblock}{4.6}(0.2,13.4)
\posterheading{BDI Plan Library}

The BDI \alert{plan library} contains plans that each resolve a goal either through primitive actions in the environment or posting sub-goals to be resolved first in a \alert{hierarchical manner}.
...
A plan is \alert{relevant} for the goal it was written to handle. It is also \alert{applicable} if it's context condition are satisfied at run-time. 
...
Second, and regardless of the above, since learning is performed while acting in the environment, then care must be taken in how much \alert{confidence} to put in the decision tree on an ongoing basis.
\end{textblock}
...
\alert{How to record training set}: We compare two approaches, a conservative BUL scheme that only records failures when all plan choices are considered well-informed, and an alternative ACL that records every outcome for learning.

\alert{How to use decision trees}: A confidence measure is applied to the decision tree prediction to calculate final plan selection weights. Confidence is proportional to the \alert{coverage} of paths below a plan.
...
The above results compare performance of\\ ACL+coverage (crosses) against BUL (circles) for various goal-plan hierarchies.
