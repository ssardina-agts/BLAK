...
agents that work with an applicability threashold, that is, agents that may not
select a plan if they are deemed not worth it. \textbf{explain more here......}


Finally, we point out that agents based on the new plan selection scheme will
be able to realize when the optimal solution for a goal has been reached before
the agents based on the previous scheme.
%%
This is because \textbf{.......... explain analytically..........}


Summarizing, the above results are significant in that they suggest that
the alternative plan selection mechanism, which uses the coverage notion as a
confidence test, does provide substantial benefits. Specifically, being \CL\ a
simpler approach, it suggests that the \CLSELB\ combination as the
overall best one.
