...
% %
Then, we calculated the average rate of success of the goal every some fixed
number of iterations (in our case $20$), and investigated how this rate evolved
as the agent refined the context condition of plans.
% %%
We ran the tests using both a \BUL-based agent and a \CL-based agent, ensuring
the same sampling of random world states for each agent.
% *** Could remove following sentence as we have already said this.
With \BUL, we used $k=3$ and $\epsilon=0.3$, 
