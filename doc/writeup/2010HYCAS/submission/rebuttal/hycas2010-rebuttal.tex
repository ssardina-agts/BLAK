...
\shout{We agree and have included references to several logistics domains used in the planning literature that are also relevant to our discussion. Where?...}
...
We believe this is adequately addressed and refer the reviewer to Section 3.4 Handling Event-Goal Types.
...
\shout{We agree and have now clarified this in page 7.}
...
In a given world state $w$ (or a set of attributes as far as the decision tree is concerned), a plan may first fail due to a poor choice of sub-plan but later pass when all correct sub-choices were made. Both outcomes will be recorded in the training set against the same world state (i.e tuples [w,F] and [w,T]). This  along with positive and negative outcomes from other world states for which the plan was invoked.

The positive and negative outcomes for various attribute value combinations (worlds) allows the decision tree induction algorithm ($J48$ from package $weka$) to filter out the attributes that carry the least information given the data. The resulting decision tree is essentially a compact representation that best explains the observations. 

If we were to only use the positive outcomes, we would simply get a look-up table (decision table) for success in given world states. However, using both negative and positive outcomes, we can build an abstract representation (decision tree) that may be used to predict outcomes even for ``as yet unseen'' worlds --- based on the values of only those world attributes that are deemed important by the representation.
...
The reviewer highlights an important issue, that of knowledge representation, and is the focus of ongoing work. Due to lack of space however, we do not delve into this in much detail apart from paragraph 2 on page 9. 

To answer your question, one way to automate the attribute set is to parse the plan bodies for this information. However, this may still miss features that are important to the decision (and would normally be included in the context condition of the plan) but are never used in the plan body. An alternative is to include the full set of attributes, however that would entail longer learning periods for collecting sufficient data to filter out the irrelevant attributes. For this work, we suggest the domain expert make the first pass at filtering out the (certainly) irrelevant attributes.
...
We believe this is addressed by the response in \ref{rev3:recursion_examples}.
...
\shout{We have updated the related work review in [Section 5. Discussion and Conclusion].}
