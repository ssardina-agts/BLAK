...
\node  (environment)  
       ...
       starburst,fill=black!10,rotate=90,shift={(-1cm,3cm)}] 
...
\draw (0,5) node (I) [anchor=south,draw, centity, text width=.5em,
	minimum height=\x em]{};
\foreach \x in {3,2,1}
\draw (0.8,5) node (I) [anchor=south,draw,centity, text width=.5em,
	minimum height=\x em]{};
\foreach \x in {5,4,3,2,1}
\draw (1.6,5) node (I) [anchor=south,draw, centity, text width=0.5em,
	minimum height=\x em]{};
