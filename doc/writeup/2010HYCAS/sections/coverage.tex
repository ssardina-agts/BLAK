...
% %
Formally, we define the plan selection weight $\Omega_P(w)$ as a function of the
\dt\ determined success expectation $\kappa_P(w)$ and the degree of coverage
$c_P(w)$:\footnote{For the \CL\ and \BUL\ schemes discussed above,
...
expectation, that is, $\Omega_P(w) = p_T(w)$.


\notems{first some text that could be of use...}
