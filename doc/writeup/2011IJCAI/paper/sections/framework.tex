...
%
Here $n$ controls the sensitivity to performance changes: smaller values make the measure more responsive while larger values give more stability.\footnote{Currently $n$ is decided by the programmer after some initial trials. This dependency could be removed in the future by using, say, a temporally reduced weighting for past executions.}
...
%
This weight definition is similar to what we have used before~\cite{singh10:extending,singh10:learning}, except for the new confidence metric $\C$ defined above. The idea is to combine the likelihood of success of plan $P$ in world $w$ (the term $\P(P,w)$) with a confidence bias (here $\C \in [0.0:1.0]$) to determine a final plan selection weight $\Omega$. 
