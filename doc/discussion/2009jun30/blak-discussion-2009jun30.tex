...
At it's core, the problem is that we are constructing a decision tree with a single sample of \emph{one} world and then using this tree to determine the probabilities for \emph{all} worlds. This is not reasonable. 
...
\item The reverse case is also a problem, where an ill-informed \dt may give an incorrect probability of $1$ for a world $W1$ it has never seen before. In this case, there is really no hope of improving in $W1$ because all siblings will be demoted to $\approx 0$ and the false-positive incorrect choice will be made $\frac{999}{1000}$ times. The situation will improve only as the  ill-informed \dt gathers more instances in other worlds and in effect remedies the problem for $W1$. However, this may not happen at all if wrong choices are always being made in a sort of \emph{deadlock}.
\end{itemize}

As a result the exploration of \st is unfairly biased leading to a slower convergence than expected in some cases, and a sub optimal (deadlocked) convergence in others. This latter case is what I think we are seeing in \tiv. Note that this problem is evident in experiments with multiple worlds, hence why we haven't come across it earlier.
...
\subsubsection{When to use a \dt?}

Section \ref{subsec:useDT} shows how the choice $m=1$ leads to ill-informed decision trees that distort plan selection probabilities. An obvious remedy is to increase $m$ to a suitable number that guarantees prediction within tolerance from the newly formed 
...
I function $useDT$ currently determines when we are ready to start using a \dt as follows:
\begin{verbatim}
...
\subsubsection{When no \dt exists, default $p$ should be $0.5$}
...
\section{A Systematic Analysis of \st}

\subsection{Test coverage}
Sebastian and I briefly discussed the \tiv result last week and agreed that it exposes some grey area about our understanding of \cc and \st. Moreover, from our current tests it is difficult to understand the conditions under which one approach performs better than the other. 
...
\subsection{Current insights into the workings of \cc and \st}
