...
public class ClassifierTree 
  ...
  public final double [] distributionForInstance(Instance instance,
    ...
    return doubles;
      ...
      /** 
       * Returns number of instances classified by the leaf
       * that will classify this instance.
       * Author: Dhirendra Singh
       *
       * @param instance the instance to get the number for
       * @return the total number of instances classified with this instance
       * @throws Exception if something goes wrong
       */
      public double countForClassification(Instance instance) throws Exception {
          double maxProb = -1;
          double currentProb;
          int maxIndex = 0;
          int j;
          ...
          return getCorrectClassifications(maxIndex, instance);
      }
  ...
  private double getProbs(int classIndex, Instance instance, double weight) 
	...
	if (son(treeIndex).m_isEmpty) {
       ...
       * @throws Exception if something goes wrong
       */
      private double getCorrectClassifications(int classIndex, Instance instance) 
      throws Exception {
          if (m_isLeaf) {
              return localModel().correctPerClass(classIndex);
          } else {
              int treeIndex = localModel().whichSubset(instance);
              if (treeIndex != -1) {
                  return son(treeIndex).getCorrectClassifications(classIndex, instance);
              } else {
                  /* Instance is assigned to more than one subtree,
                   * when attribute is missing.
                   * So take average of all instances in those subtrees.
                   * Todo.
                   */
              }
          }
